...
##                  % (name, symtype, seg, val, size), file=sys.stderr)
        if name.startswith("@"):
            namewithscope = cheapscope + "." + name[1:]
        else:
            scopeprefix = scopes[scope] if scope else ""
            scopeprefix + scopeprefix + "." if scopeprefix else ""
            namewithscope = cheapscope = scopeprefix + name
        seg = segs[seg]
