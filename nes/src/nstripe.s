...
; @param Y low byte of destination address in VRAM
; @param nstripe_src* source address, usually in ROM
.proc nstripe_append_128_to_ay
  ; Save destination address
  pha
  sty nstripe_left
  jsr nstripe_append_64_to_ay
  ...
  ; Restore destination address to second half
  lda nstripe_left
  adc #64
  tay
  pla
...
;;
; Appends a 64-byte literal to the update buffer.
; @param A high byte of destination address in VRAM
; @param Y low byte of destination address in VRAM
; @param nstripe_src* source address, usually in ROM
.proc nstripe_append_64_to_ay
  ldx popslide_used
